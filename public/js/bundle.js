...
var displayMap = function displayMap(locations) {
  mapboxgl.accessToken = 'pk.eyJ1IjoiaXNoYW52aiIsImEiOiJjazJnbjlleGMwMHdkM2pvMHMyaTZ3cnY5In0.94RG5TlegDMwpIKEShlrjQ';
  var map = new mapboxgl.Map({
    container: 'map',
    // define id to be replaced here
      ...
      while (1) {
        switch (_context2.prev = _context2.next) {
          ...
          case 3:
            res = _context2.sent;
            // for Reload
            if (res.data.status === 'success') location.reload(true); // if not set to true it will reload from cache but we want reload from server
...
var stripe = Stripe('pk_test_fmFlbNbkOkxOVoeQYpIDUzPl00ZWiYlZN5');
...
if (userDataForm) {
  userDataForm.addEventListener('submit', function (e) {
    e.preventDefault(); // telling that it is multipart form-data encoding
      ...
      return regeneratorRuntime.wrap(function _callee$(_context) {
        while (1) {
          switch (_context.prev = _context.next) {
            ...
            case 7:
              document.querySelector('.btn--save-password').textContent = 'Save Password'; // deleting password once updated from forms
...
if (bookBtn) {
  bookBtn.addEventListener('click', function (e) {
    ...
    var tourId = e.target.dataset.tourId; //e.target is button that fired the event and data is stored in dataset.tourId we use destructuring
